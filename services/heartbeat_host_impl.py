#!/usr/bin/env python3
"""Durable Master Records host for the canonical StegVerse single heartbeat.

The host is replaceable infrastructure only. Canonical heartbeat code is cloned
from StegVerse-Labs/.github at process start. Mutable runtime state is restored
from and checkpointed to the dedicated Master Records Render Key Value store.
Neither Render nor this host grants execution authority or owns heartbeat cadence.
"""
...
health: dict[str, Any] = {
    ...
    "active_claims": {},
    "active_fencing_tokens": {},
    "duplicate_claim_ids": [],
    "duplicate_fencing_tokens": [],
...
def merge_registry(root: Path, persisted_payload: str) -> None:
    ...
    for task_id in sorted(set(current_tasks) | set(persisted_tasks)):
        if task_id in current_tasks and task_id in persisted_tasks:
            merged = dict(current_tasks[task_id])
            merged.update(persisted_tasks[task_id])
            tasks.append(merged)
        else:
...
def registry_summary(root: Path) -> dict[str, Any]:
    ...
    for task in registry.get("tasks", []):
        ...
        if task_id and isinstance(claim_id, str) and claim_id:
            claims[task_id] = claim_id
        timing = task.get("heartbeat_timing") or {}
        fence = timing.get("fencing_token")
        if task_id and isinstance(fence, int):
            fences[task_id] = fence
    claim_counts = Counter(claims.values())
    fence_counts = Counter(fences.values())
    return {
        "registry_generation": int(registry.get("generation", 0)),
        "active_claims": dict(sorted(claims.items())),
        "active_fencing_tokens": dict(sorted(fences.items())),
        "duplicate_claim_ids": sorted(value for value, count in claim_counts.items() if count > 1),
        "duplicate_fencing_tokens": sorted(value for value, count in fence_counts.items() if count > 1),
    }
...
def persist_snapshot(client: redis.Redis, source_sha: str) -> dict[str, Any]:
    ...
    health.update(registry_summary(WORK_ROOT))
    return snapshot
